#' Performs conditional meta-analysis of coding functional categories by using the MetaSTAARlite pipeline.
#'
#' This function performs meta-analysis to detect associations between a
#' quantitative/dichotomous phenotype and coding functional categories of a gene by using by using the MetaSTAARlite pipeline.
...
#' @param coding_cov_cond_gene_list a list containing the names of the variants to be conditioned on
...
#' specified in \code{\link{coding_incl_ptv_MetaSTAARlite_worker}} (default = TRUE).
...
#' @export
coding_incl_ptv_MetaSTAARlite_cond <- function(chr,gene_name,genes,
  ...
  if(!is.null(results_ptv_ds))
  {
    ...
    colnames(results_ptv_ds)[1:4] <- c("Gene name","Chr","Category","#SNV")
    colnames(results_ptv_ds)[(dim(results_ptv_ds)[2]-1):dim(results_ptv_ds)[2]] <- c("ACAT-O-MS","MetaSTAAR-O")
  }
  ...
  if(!is.null(results_ptv))
  {
    ...
    colnames(results_ptv)[1:4] <- c("Gene name","Chr","Category","#SNV")
    colnames(results_ptv)[(dim(results_ptv)[2]-1):dim(results_ptv)[2]] <- c("ACAT-O-MS","MetaSTAAR-O")
  }
  ...
  results_coding <- list(plof=results_plof,ptv=results_ptv,plof_ds=results_plof_ds,ptv_ds=results_ptv_ds,missense=results,disruptive_missense=results_ds,synonymous=results_synonymous)
